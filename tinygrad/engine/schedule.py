...
def _lower_lazybuffer(outs:List[LazyBuffer], realizes:Dict[LazyBuffer, None]) -> Tuple[ScheduleItem, Dict[Variable, int]]:
  ...
  if (out:=outs[0]).op in {MetaOps.CUSTOM, MetaOps.COPY, MetaOps.EMPTY, MetaOps.VIEW}:
    return ScheduleItem(UOp(UOps.EXT, out.dtype, (), (out.op, out.arg)), tuple(x.base.buffer for x in (out,)+out.srcs if x.base.buffer.size != 0)), {}
  # create the stores
  ...
  if AST_REWRITE:
    try: sink = graph_rewrite(sink, reduceop_fusor)
      ...
      graph_uop(sink)
      raise e
  return ScheduleItem(sink, tuple(x.base.buffer for x in outs+list(inputs) if x.base.buffer.size != 0),
